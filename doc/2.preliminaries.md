...
本文也是综合式 (synthetic) 递归论的前置知识, 将来计划写的 Coq 递归论也将引用此章.
...
我们不想像传统教科书那样执着于单字母命名, 也不想像Java那样使用冗长的驼峰式命名. 我们将在两者之间寻找一种平衡, 为此, 我们将视情况实验性地使用表意性高的汉字命名. 此外, 我们的代码将取代 Latex 公式, 直接用于展示目的. 因此, 我们格外注重可读性, 而不惜放弃输入的便利性. 为此, 我们将积极使用各种 Unicode 符号, 而不使用 ASCII 组合符号.

我们将导入 [Utf8_core](https://coq.inria.fr/library/Coq.Unicode.Utf8_core.html) 的记法, 将自然数类型 [nat](https://coq.inria.fr/library/Coq.Init.Datatypes.html#nat) 简写作 ℕ, 并按主流语言的惯例用问号表示[可选类型](https://coq.inria.fr/library/Coq.Init.Datatypes.html#option).
...
我们仿照存在量词的记法表示 [Σ类型](https://coq.inria.fr/library/Coq.Init.Specif.html#sigT).
...
在 Coq 中, 谓词也是函数, 我们上面定义的函数外延就已经覆盖了谓词外延的情形. 我们来看一个例子. 我们知道康托尔定理是说不存在 ℕ 到 ℕ → ℕ 的满射. 我们来看一个在 Coq 中相当容易证明的变体: 给定类型 T, 不存在 T 到其谓词的满射. 也就是说, 对 T 到其谓词的任意函数 f : T → (T → Prop), 并非对任意谓词 p : T → Prop, 都存在一个参数 x : T, 使得 f x 与 p 外延相等. 此处的外延相等可以写作 f x ≡{T → Prop} p, 也可以简写作 f x ≡{_} p, Coq 可以推断出省略的部分. 证明是相当直接的, 不再赘述.
...
我们将 F (x, y) 记作 ⟨x, y⟩, 将 G n 记作 ⎞n⎛. 用此记法, 我们有
- ⎞⟨x, y⟩⎛ = (x, y)
- ⟨fst ⎞n⎛, snd ⎞n⎛⟩ = n
...
我们仿照边绑定边模式匹配的记法 λ '(x, y), 定义边绑定边转码的记法 λ' ⟨x, y⟩
...
我们知道 Coq 中没有一般性的大消除 (large elimination), 即通常情况下不能从 ∃ x, p x 得到 Σ x, p x. 但对于某些类型, 在满足一定条件下, 可以有大消除. 自然数就是一个例子. 而且我们不单可以消除, 由自然数的良基性, 我们还可以提取出**最小**的那个.
...
仔细观察可以发现其语义像是: 存在 n ≤ m 使得 f m = true. 但要注意它跟∃句在形式上有本质区别, 虽然语义上非常相似. 要建立 G n, 必须先建立 G (S n), 以此类推, 一路往上, 直到遇到某 m 使得 f m = true 为止. 这实际上是一个计算的过程, 而这是∃句所不具有的, 因此可以成为沟通∃和Σ的桥梁.
...
给定了强逻辑可判定条件 dec, 就可以构造出对于的 bool 函数  
...
我们需要给上一小节可枚举性的定义一些辩护. 在传统教科书中, 递归可枚举性是用可计算的偏函数定义的. 我们在这里无法这么做, 因为类型论中可定义的函数都是全函数. 我们也不能用函数关系 R : A → B → Prop 来建模, 因为这会囊括进不可计算函数. 因此我们采用了可选序列, 但它只是偏函数的其中一个模型. 为了后续证明可枚举和可判定的概念的定义与偏函数模型的选择无关, 我们在此处先建立一个抽象的偏函数结构.
...
最后3条规则将使"偏 A"成为一个单子 (monad), 假设读者都有函数式编程的背景, 不解释.
...
可以证明安定的可选序列就是偏函数的一个模型. 具体而言, 可选序列即步进求值器, 我们将它与安定性条件合在一起, 定义为"偏 A"封装.
...
给定类型A, 解包关系 x ?= a 则定义成求值序列中 x @ n 中有 a.
...
可以证明, 当 f 非空时, μ f 有值. 用上文的自然数大消除及其最小性即证, 当然这里并不需要其"大".
...
左到右的证明关键是用枚举器 f 构造偏函数 (λ n, if f n is Some x then 有 x else 无), 右到左的证明关键是用偏函数模型里的求值部件, 将偏函数 f 改造成枚举器 (λ' ⟨n, m⟩, 求值 (f n) m), 这里用了 ℕ 到 ℕ×ℕ 的转码.
